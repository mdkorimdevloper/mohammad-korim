(function(){
  ...
  const ready=(async()=>{
    try{
      ...
      window.__sharedSupabase=client;window.__supabaseReady=Promise.resolve(client);return client;
  ...
  function applyImage(){
    document.querySelectorAll('.hero-img img,.about-img,.team-avatar,.profile-image img,img[data-profile-image],img.profile-img,img.avatar').forEach(img=>{
      if(img.src!==NEW_PROFILE_IMAGE){img.src=NEW_PROFILE_IMAGE;img.removeAttribute('srcset');}
  ...
  function initRequestedDesign(){
    ...
    const footerTitle=document.querySelector('.footer-brand h3');if(footerTitle)footerTitle.textContent="I'm MD Korim";
    const footerBottom=document.querySelector('.footer-bottom');if(footerBottom){const spans=footerBottom.querySelectorAll('span');if(spans.length>1)spans[1].textContent="I'm MD Korim";}
    applyImage();
  }
  ready.then(applyImage).catch(console.error);
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{initRequestedDesign();applyImage();});else{initRequestedDesign();applyImage();}
